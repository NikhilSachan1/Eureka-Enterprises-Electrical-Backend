...
export class PendingSettlementQueryDto {
  ...
  @ApiProperty({ description: 'Start date filter on expenseDate (YYYY-MM-DD)', required: false })
  @IsOptional()
  @IsDateString()
  startDate?: string;

  @ApiProperty({ description: 'End date filter on expenseDate (YYYY-MM-DD)', required: false })
  @IsOptional()
  @IsDateString()
  endDate?: string;
