...
export const buildVehicleEventsQuery = ({
  ...
  const dataQuery = `
    SELECT 
      ...
      -- From user details (previous holder - for REJECT use toUser=A since fromUser=B is the rejector)
      CASE
        WHEN ve."eventType" = '${VehicleEventTypes.HANDOVER_REJECTED}' THEN
