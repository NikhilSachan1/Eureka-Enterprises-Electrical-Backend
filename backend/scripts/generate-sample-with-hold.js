...
function numberToWords(amount) {
  ...
  let n=Math.floor(amount); let r='';
  if(n>=10000000){r+=toWords(Math.floor(n/10000000)).trim()+' Crore ';n=n%10000000;}
  if(n>=100000){r+=toWords(Math.floor(n/100000)).trim()+' Lakh ';n=n%100000;}
  if(n>=1000){r+=toWords(Math.floor(n/1000)).trim()+' Thousand ';n=n%1000;}
  r+=toWords(n);
...
(async() => {
  const d = {
    referenceNumber: 'EE/TA/2526/002',
    ...
    utrNumber: 'HDFC2509010054321',
    transferAmount: 480000,
    taxableAmount: 500000,
    tdsDeductionAmount: 20000,
    paymentTotalAmount: 480000,
    paymentHoldReason: 'Payment held pending submission of GST invoice copy.',
    invoiceNumber: 'SC/INV/2025-26/048',
  ...
  const gross = Number(d.taxableAmount);
  const tds = Number(d.tdsDeductionAmount);
  const net = Number(d.paymentTotalAmount);
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header .company { font-size: 17px; font-weight: bold; text-transform: uppercase; letter-spacing: .5px; }
...
<div class="page">
    ...
    <tr>
      ...
      <td class="info-cell" style="width:49%">
        <table>
          ...
          <tr><td class="lbl">Date</td><td class="sep">:</td><td class="val">${fmtDate(d.generatedAt)}</td></tr>
          ...
          <tr><td class="lbl">Financial Year</td><td class="sep">:</td><td class="val">FY 25-26</td></tr>
        </table>
      </td>
    </tr>
  </table>
  ...
  <div class="payment-msg">
    ...
    (${numberToWords(d.transferAmount)}).
  </div>

  <div class="hold-reason">
    <strong>Payment Hold Reason:</strong> ${d.paymentHoldReason}
  </div>
  ...
  <table>
    ...
    <tbody>
      <tr>
        ...
        <td class="r">${fmtN(gross)}</td>
        <td class="r">${fmtN(tds)}</td>
        <td class="r">${fmtN(net)}</td>
      </tr>
    </tbody>
  </table>
  ...
  const buf = await page.pdf({ format: 'A4', printBackground: true, margin: { top: '20px', bottom: '20px', left: '30px', right: '30px' } });
  const outPath = 'C:/Users/Coditas-Admin/Desktop/sample-payment-advice-with-hold.pdf';
